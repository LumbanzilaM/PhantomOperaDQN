...
class PurplePowerEnvManager(AEnvManager):
    ...
    def learn(self, env, is_end):
        if not self.smart:
            ...
            print("Purple Power first env ", self.env[0])
            print("Purple Power last env ", self.env[1])
            print("suspects ", self.suspect_nbr)
            print("Purple Power reward ", self.reward)
